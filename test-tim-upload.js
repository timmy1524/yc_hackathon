// Test client-upload with Tim's real files
const fs = require('fs');

async function testTimUpload() {
  try {
    console.log('🎵 Testing client-upload with Tim\'s files...')
    ...
    // Create request body with Tim's data
    const requestBody = {
    ...
    console.log('🚀 Calling client-upload function...')
    ...
    if (response.ok) {
      ...
      try {
        ...
        // Wait a moment then check the contacts
        console.log('\n🔍 Checking contacts after upload...')
        setTimeout(async () => {
          try {
            const contactsResponse = await fetch('https://shktirpoweaqcvvleldo.supabase.co/functions/v1/chrome-get-contacts', {
              method: 'GET',
              headers: {
                'Content-Type': 'application/json',
                'Authorization': 'Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InNoa3RpcnBvd2VhcWN2dmxlbGRvIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NTk1OTQ4MzMsImV4cCI6MjA3NTE3MDgzM30.SJDU3hDL4N7jbhT7Kqp6JuNKjIXWAG3nKMoMk5wuz8w'
